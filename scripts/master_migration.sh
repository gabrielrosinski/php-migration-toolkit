...
mkdir -p "$OUTPUT_DIR"/{analysis,services,prompts,logs,database}
...
phase3_database() {
    ...
    # Generate condensed schema summary and module-specific schemas
    if [ -f "$OUTPUT_DIR/database/schema_inferred.json" ]; then
        echo ""
        run_with_spinner "Generating condensed schema summary" 30 \
            python3 "$SCRIPT_DIR/generate_schema_summary.py" \
                "$OUTPUT_DIR/database/schema_inferred.json" \
                -o "$OUTPUT_DIR/database/schema_summary.json" \
                --all-modules || {
                    echo -e "  ${YELLOW}⚠ Schema summary generation failed (non-fatal)${NC}"
                }

        if [ -f "$OUTPUT_DIR/database/schema_summary.json" ]; then
            SUMMARY_SIZE=$(du -k "$OUTPUT_DIR/database/schema_summary.json" | cut -f1)
            MODULE_COUNT=$(ls -1 "$OUTPUT_DIR/database/modules/"schema_*.json 2>/dev/null | wc -l | tr -d ' ')
            echo -e "  ${GREEN}✓${NC} Generated schema summary (${SUMMARY_SIZE}KB) + ${MODULE_COUNT} module schemas"
        fi
    fi
...
phase6_design() {
    ...
    # Check if ARCHITECTURE.md already exists (from previous run)
    if [ -f "$OUTPUT_DIR/analysis/ARCHITECTURE.md" ]; then
        echo -e "  ${GREEN}✓${NC} ARCHITECTURE.md already exists"
    fi
...
phase7_generation() {
    ...
    if [ -f "$TOOLKIT_ROOT/migration-steps.md" ]; then
        echo -e "  ${GREEN}See migration-steps.md for all service migration commands${NC}"
        echo "  Commands are organized by phase with specific prompts for each module."
    else
        echo "  After completing system design, see prompts/migration/ for module-specific prompts."
    fi
    echo ""
...
phase8_testing() {
    ...
    # Check if migration-steps.md exists
    if [ -f "$TOOLKIT_ROOT/migration-steps.md" ]; then
        echo -e "  ${GREEN}See migration-steps.md Phase 4 for validation commands${NC}"
    else
        echo "  After generating services, see prompts/full_validation.md"
    fi
    echo ""
...
summary() {
    ...
    if [ -f "$TOOLKIT_ROOT/output/analysis/ARCHITECTURE.md" ] && [ -f "$TOOLKIT_ROOT/output/analysis/NESTJS_BEST_PRACTICES.md" ]; then
        echo "2. ARCHITECTURE DESIGN"
        echo -e "   ${GREEN}✓ Already complete:${NC}"
        echo "   - output/analysis/ARCHITECTURE.md"
        echo "   - output/analysis/NESTJS_BEST_PRACTICES.md"
        echo ""
    else
        echo "2. SYSTEM DESIGN (if not already done)"
        echo "   See: prompts/system_design_architect.md"
        echo "   Output: output/analysis/ARCHITECTURE.md, output/analysis/NESTJS_BEST_PRACTICES.md"
        echo ""
    fi

    echo "3. CREATE NX WORKSPACE"
    echo -e "   ${CYAN}./scripts/create_nx_workspace.sh -o ./output${NC}"
    echo ""

    # Check if migration-steps.md exists
    if [ -f "$TOOLKIT_ROOT/migration-steps.md" ]; then
        echo "4. MIGRATE SERVICES"
        echo -e "   ${GREEN}See migration-steps.md for all Ralph Wiggum commands${NC}"
        echo "   Commands are organized by phase with iteration estimates"
        echo ""
    else
        echo "4. MIGRATE SERVICES"
        echo "   See prompts/migration/ for module-specific prompts"
        echo ""
    fi

    echo "5. VALIDATION"
    echo "   See: prompts/full_validation.md"
    echo ""
