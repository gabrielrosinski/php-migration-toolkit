...
echo ""
echo "  5. Migrate services using Ralph Wiggum:"
echo -e "     ${CYAN}/ralph-loop \"\$(cat $OUTPUT_DIR/../prompts/legacy_php_migration.md)\" \\${NC}"
echo -e "     ${CYAN}  --completion-promise \"SERVICE_COMPLETE\"${NC}"
echo ""
if [ ${#SERVICES[@]} -gt 0 ]; then
    echo "  6. For each extracted microservice:"
    echo -e "     ${CYAN}/ralph-loop \"\$(cat $OUTPUT_DIR/../prompts/extract_service.md)\" \\${NC}"
    echo -e "     ${CYAN}  --context $OUTPUT_DIR/services/{service}/analysis/service_context.json${NC}"
    echo ""
fi
