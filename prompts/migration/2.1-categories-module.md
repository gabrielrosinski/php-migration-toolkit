...
| `output/database/modules/schema_categories.json` | Module-specific schema (condensed) |
...
target_files = ['cats/index.php', 'catsX/index.php']
...
for r in data.get('routes', []):
    ...
    if any(t in target for t in target_files) or 'category' in path.lower() or path == '/cats':
        found_any = True
...
- [ ] All routes that belong to Categories
...
```
Read tool parameters:
  file_path: output/database/modules/schema_categories.json
```

This file contains only the tables relevant to categories (condensed format, ~2.7KB).

**Tables included:** z_item, z_main, ntag_name, ntag_parts, api_world, z_carousel
...
|-------------|------------|-----|---------------|--------------|-------|
| normalizeSelect | $input | No | normalizeSelect | Sanitize input | Input normalization |
| buildSeoData | $result, $data... | Yes | buildSeoData | Parameterized queries | SEO metadata |
| cleanSearchWord | $word | No | cleanSearchWord | XSS sanitization | Search input |
| blackFriday | $select, $world | No | getBlackFridayData | - | Promotions |
| getWorldBySelect | $select | Yes | getWorldBySelect | Parameterized queries | World lookup |
| ... | ... | ... | ... | ... | ... |
...
|--------------|--------------|-------------|
| GET /cats | GET /cats | All categories |
| GET /category/:id | GET /category/:id | Category by ID with products |
| GET /categoryX/:id | GET /categoryX/:id | Extended category view |

### Entities to Use

- `Category` - Main category entity
- `ZItem` - Category items
- `ZMain` - Main category data
- `NtagName` - Tag names
- `NtagParts` - Tag-product relationships
...
### Target Location
- Module: `apps/gateway/src/modules/categories/`
...
- [ ] **Step 1.7: READ schema_categories.json** (module-specific, 6 tables)
...
### Validation Commands
```bash
npx nx test gateway --testPathPattern="categories"
