...
| File | Purpose |
|------|---------|
| `output/database/entities/ntag-name.entity.ts` | Brand/tag names (where up_uin = 21 for brands) |
| `output/database/entities/parts.entity.ts` | Products by brand |
...
target_files = ['routes/brands/index.php', 'routes/shop/index.php', 'files/brands.php', 'files/shop.php']
...
target_files = ['brands', 'shop']

for r in data.get('routes', []):
    target = r.get('target_file', '')
    path = r.get('nestjs_path', '')
    if any(t in target for t in target_files) or any(t in path.lower() for t in ['brand', 'shop']):
...
- [ ] All routes that belong to brands
...
```
Read tool parameters:
  file_path: output/database/modules/schema_brands.json
```
...
|-------------|------------|-----|---------------|----------------|
| getBrandById | $id | Yes | getBrandById | Brand by ID |
| getShopBySlug | $slug | Yes | getShopBySlug | Shop/brand page |
| getShopCategory | $slug, $cat | Yes | getShopCategory | Shop category |
| getBrandProducts | $brandId, $limit | Yes | getBrandProducts | Products by brand |
| getAllBrands | - | Yes | getAllBrands | All brands list |
| getBrandSeo | $brandId | Yes | getBrandSeo | Brand SEO data |
| ... | ... | ... | ... | ... |

**STOP:** Do not proceed until you have mapped ALL functions from brand-related PHP files.
...
|--------------|--------------|-------------|
| GET /brands/:id | GET /brands/:id | Brand by ID |
| GET /shop/:slug | GET /shop/:slug | Shop/brand page |
| GET /shop/:slug/category/:cat | GET /shop/:slug/category/:cat | Shop category |

### Entities to Use (from libs/database/src/entities/)

- `NtagName` - Brand/tag names (where up_uin = 21 for brands)
- `Parts` - Products by brand
...
### Target Location
- Module: `apps/gateway/src/modules/brands/`
...
3. **DTOs**: Create validated DTOs for brand data
4. **Caching**: Add caching for brand information
5. **SEO**: Support brand-specific SEO data
6. **Tests**: Unit tests with >80% coverage

### Key PHP Functions to Implement

From brand-related PHP files:
- `getBrandById($id)` - Brand by ID [DB]
- `getShopBySlug($slug)` - Shop page [DB]
- `getShopCategory($slug, $cat)` - Shop category [DB]
- `getBrandProducts($brandId, $limit)` - Brand products [DB]
- `getAllBrands()` - All brands [DB]
- `getBrandSeo($brandId)` - Brand SEO [DB]
...
### Validation Commands
```bash
npx nx test gateway --testPathPattern="brands"
