# User Settings Module Migration

## CRITICAL: Analysis-First Approach

**DO NOT START IMPLEMENTATION** until you have completed the mandatory analysis phase.

**FAILURE CONDITIONS - Migration will be rejected if:**
- You skip reading ARCHITECTURE.md sections with the Read tool
- You skip reading NESTJS_BEST_PRACTICES.md with the Read tool
- You don't create a function mapping table
- Security requirements are not applied

---

## DATA SOURCES (Must Read Before Implementation)

### Primary Analysis Files (in output/ folder)
| File | Purpose | What to Extract |
|------|---------|--------------------|
| `output/analysis/legacy_analysis.json` | PHP function analysis | Functions, parameters, DB calls, globals, complexity |
| `output/analysis/routes.json` | Route mappings | HTTP method, NestJS path, query params, target file |
| `output/analysis/architecture_security_db.json` | Security issues | XSS, SQL injection, command injection to fix |

### Architecture Reference (MUST READ WITH Read TOOL)
| File | Section | Line Range | Required Extractions |
|------|---------|------------|---------------------|
| `ARCHITECTURE.md` | **Section 9: Route Mapping** | Lines 820-970 | Routes for this module |
| `ARCHITECTURE.md` | **Section 12: Security** | Lines 1248-1340 | crypto.randomBytes, bcrypt, validation |
| `NESTJS_BEST_PRACTICES.md` | **Full file** | All | Patterns, decorators, testing |

### Database (Import entities, don't recreate)
| File | Purpose |
|------|---------|
| `output/database/entities/app-devices.entity.ts` | User devices |
| `output/database/entities/price-down.entity.ts` | Price alerts |
| `output/database/entities/item-down.entity.ts` | Stock alerts |
| `output/database/entities/return-items.entity.ts` | Return notifications |
| `output/database/entities/api-personal-items.entity.ts` | Personal items |

---

## PHASE 1: MANDATORY ANALYSIS (Must Complete Before Writing Code)

### Step 1.1: Read PHP Source Files Analysis

Execute this command to extract the PHP functions for this module:

```bash
cat output/analysis/legacy_analysis.json | python3 -c "
import json, sys
data = json.load(sys.stdin)

target_files = ['routes/set/index.php', 'routes/set/priceAlert/index.php', 'routes/set/stockAlert/index.php', 'routes/set/setIdComputer/index.php', 'routes/set/city/index.php', 'routes/set/app/index.php', 'routes/user/index.php', 'files/settings.php', 'files/user.php']

all_files = data.get('all_files', [])
for f in all_files:
    path = f.get('path', '')
    if any(t in path for t in target_files):
        print(f'\\n=== {path.split(\"m-action/\")[-1]} ===')
        print(f'Lines: {f.get(\"total_lines\")}')
        for fn in f.get('functions', []):
            params = ', '.join(fn.get('params', []))
            db = '[DB]' if fn.get('calls_db') else ''
            globals_used = fn.get('uses_globals', [])
            print(f'  - {fn.get(\"name\")}({params}) {db}')
            if globals_used:
                print(f'    Globals: {globals_used}')
"
```

### Step 1.2: Read Routes Analysis

```bash
cat output/analysis/routes.json | python3 -c "
import json, sys
data = json.load(sys.stdin)

target_files = ['set/', 'user', 'priceAlert', 'stockAlert', 'setIdComputer', 'city', 'app/cookie', 'app/device']

for r in data.get('routes', []):
    target = r.get('target_file', '')
    path = r.get('nestjs_path', '')
    if any(t in target for t in target_files) or '/set/' in path or '/user' in path:
        print(f\"{r.get('nestjs_method')} {r.get('nestjs_path')} -> {target}\")
        if r.get('query_params'):
            print(f\"  Query params: {r.get('query_params')}\")
"
```

### Step 1.3: Read Security Issues

**IMPORTANT:** Security issues affect modules even if files aren't named for this module.

```bash
cat output/analysis/architecture_security_db.json | python3 -c "
import json, sys
data = json.load(sys.stdin)

# Show ALL security types relevant to backend modules
relevant_types = ['xss', 'sql_injection', 'command_injection', 'weak_crypto', 'insecure_function']

print('=== SECURITY ISSUES TO ADDRESS ===')
print()
for issue_type in relevant_types:
    details = data.get('security', {}).get('by_type', {}).get(issue_type, {})
    affected = details.get('affected_files', [])
    if affected:
        print(f'{issue_type.upper()} ({len(affected)} files):')
        for f in affected[:3]:
            print(f'  - {f}')
        if len(affected) > 3:
            print(f'  ... and {len(affected) - 3} more')
        if details.get('examples'):
            ex = details['examples'][0]
            print(f'  Remediation: {ex.get(\"recommendation\", \"N/A\")}')
        print()
"
```

### Step 1.4: READ ARCHITECTURE.md Section 9 - Routes (MANDATORY)

**YOU MUST USE THE Read TOOL** to read this section:

```
Read tool parameters:
  file_path: output/analysis/ARCHITECTURE.md
  offset: 820
  limit: 150
```

**Extract and document for this module:**
- [ ] All routes that belong to user-settings
- [ ] HTTP methods (GET, POST, PUT, DELETE)
- [ ] Path parameters and query parameters
- [ ] Response formats

### Step 1.5: READ ARCHITECTURE.md Section 12 - Security (MANDATORY)

**YOU MUST USE THE Read TOOL** to read this section:

```
Read tool parameters:
  file_path: output/analysis/ARCHITECTURE.md
  offset: 1248
  limit: 95
```

**Extract and document:**
- [ ] Use `crypto.randomBytes()` for tokens (NOT Math.random())
- [ ] Use `bcrypt` for password hashing
- [ ] Use TypeORM parameterized queries (NOT string concatenation)
- [ ] Sanitize all user inputs

### Step 1.6: READ NESTJS_BEST_PRACTICES.md (MANDATORY)

**YOU MUST USE THE Read TOOL** to read the relevant section:

```
Read tool parameters:
  file_path: output/analysis/NESTJS_BEST_PRACTICES.md
  offset: 1
  limit: 300
```

**Extract patterns for:**
- [ ] Module structure
- [ ] Service patterns
- [ ] Controller patterns
- [ ] DTO validation
- [ ] Repository usage

### Step 1.7: Read Database Schema (MANDATORY)

**YOU MUST USE THE Read TOOL:**

```
Read tool parameters:
  file_path: output/database/modules/schema_user-settings.json
```

**Extract tables relevant to this module.**

### Step 1.8: Create Function Mapping Table

**YOU MUST CREATE THIS TABLE** based on the analysis output above:

| PHP Function | Parameters | DB? | NestJS Method | Business Logic |
|-------------|------------|-----|---------------|----------------|
| setPriceAlert | $productId, $userId, $targetPrice | Yes | setPriceAlert | Set price alert |
| setStockAlert | $productId, $userId | Yes | setStockAlert | Set stock alert |
| setIdComputer | $deviceId | Yes | setDeviceId | Set device ID |
| setCity | $cityId | Yes | setCity | Set city preference |
| setAppCookie | $data | Yes | setAppCookie | App cookie settings |
| registerDevice | $token, $platform | Yes | registerDevice | Device registration |
| getUserInfo | $userId | Yes | getUserInfo | Get user info |
| getUserPreferences | $userId | Yes | getUserPreferences | User preferences |
| setDebug | $enabled | No | setDebug | Debug settings |
| ... | ... | ... | ... | ... |

**STOP:** Do not proceed until you have mapped ALL functions from user settings-related PHP files.

---

## PHASE 2: ARCHITECTURE CONTEXT

### Routes to Implement (from ARCHITECTURE.md Section 9)

| Legacy Route | NestJS Route | Description |
|--------------|--------------|-------------|
| GET /set/priceAlert | GET /set/priceAlert | Set price alert |
| GET /set/stockAlert | GET /set/stockAlert | Set stock alert |
| GET /set/setIdComputer | GET /set/setIdComputer | Set device ID |
| GET /set/debug | GET /set/debug | Debug settings |
| GET /set/city/:id | GET /set/city/:id | Set city preference |
| GET /set/app/cookie | GET /set/app/cookie | App cookie settings |
| GET /set/app/device | GET /set/app/device | Device registration |
| GET /user | GET /user | Get user info |

### Entities to Use (from libs/database/src/entities/)

- `AppDevices` - User devices
- `PriceDown` - Price alerts
- `ItemDown` - Stock alerts
- `ReturnItems` - Return notifications
- `ApiPersonalItems` - Personal items

---

## PHASE 3: IMPLEMENTATION

**Only proceed after completing PHASE 1 and PHASE 2.**

### Target Location
- Module: `apps/gateway/src/modules/user-settings/`
- Project: `m-action-nestjs`

### Implementation Requirements

1. **Service Methods**: Implement ALL PHP functions from Phase 1 mapping
2. **Controller Routes**: Map ALL routes from Phase 2
3. **DTOs**: Create validated DTOs for settings operations
4. **Auth**: Integrate with JWT auth for user identification
5. **Caching**: Add caching for user preferences
6. **Tests**: Unit tests with >80% coverage

### Key PHP Functions to Implement

From user settings-related PHP files:
- `setPriceAlert($productId, $userId, $targetPrice)` - Price alert [DB]
- `setStockAlert($productId, $userId)` - Stock alert [DB]
- `setIdComputer($deviceId)` - Device ID [DB]
- `setCity($cityId)` - City preference [DB]
- `setAppCookie($data)` - App cookie [DB]
- `registerDevice($token, $platform)` - Device registration [DB]
- `getUserInfo($userId)` - User info [DB]
- `getUserPreferences($userId)` - Preferences [DB]
- `setDebug($enabled)` - Debug settings
- ... (see Step 1.1 output for full list)

---

## PHASE 4: VERIFICATION

### Pre-Implementation Checklist

**Analysis Steps (ALL REQUIRED):**
- [ ] Step 1.1: Executed PHP function extraction
- [ ] Step 1.2: Executed routes extraction
- [ ] Step 1.3: Executed security issues extraction
- [ ] **Step 1.4: READ ARCHITECTURE.md Section 9** (routes)
- [ ] **Step 1.5: READ ARCHITECTURE.md Section 12** (security)
- [ ] **Step 1.6: READ NESTJS_BEST_PRACTICES.md**
- [ ] **Step 1.7: READ schema_inferred.json**
- [ ] Step 1.8: Created complete function mapping table

**Understanding Verified:**
- [ ] All routes for this module identified
- [ ] All database tables identified
- [ ] Security requirements documented
- [ ] NestJS patterns understood

### Post-Implementation Checklist

- [ ] All routes mapped in controller
- [ ] All PHP functions have NestJS equivalents
- [ ] DTOs have proper validation (`class-validator`)
- [ ] Entities imported from `@m-action-nestjs/database`
- [ ] Security fixes applied:
  - [ ] `crypto.randomBytes()` used (NOT Math.random())
  - [ ] TypeORM parameterized queries (NO string SQL)
  - [ ] Input sanitization applied
- [ ] Tests pass with >80% coverage

### Validation Commands
```bash
npx nx test gateway --testPathPattern="user-settings"
npx nx build gateway
```

---

## COMPLETION

When all verifications pass:
```
SERVICE_COMPLETE
```

If stuck after analysis:
```
NEEDS_REVIEW: [describe the blocking issue]
```
