...
| File | Purpose |
|------|---------|
| `output/database/entities/n-comp-clienta.entity.ts` | Client comparisons |
| `output/database/entities/n-comp-row.entity.ts` | Comparison rows |
| `output/database/entities/n-comp-template.entity.ts` | Comparison templates |
| `output/database/entities/parts.entity.ts` | Products to compare |
...
target_files = ['routes/compare/index.php', 'routes/comp/index.php', 'files/compare.php', 'files/comp.php']
...
target_files = ['compare', 'comp']

for r in data.get('routes', []):
    target = r.get('target_file', '')
    path = r.get('nestjs_path', '')
    if any(t in target for t in target_files) or 'comp' in path.lower():
...
- [ ] All routes that belong to compare
...
```
Read tool parameters:
  file_path: output/database/modules/schema_compare.json
```
...
|-------------|------------|-----|---------------|----------------|
| getComparison | $productIds | Yes | getComparison | Compare products |
| getComparisonById | $id | Yes | getComparisonById | Comparison by ID |
| createComparison | $userId, $productIds | Yes | createComparison | Create new comparison |
| addToComparison | $compId, $productId | Yes | addToComparison | Add product to comparison |
| removeFromComparison | $compId, $productId | Yes | removeFromComparison | Remove from comparison |
| getComparisonTemplate | $categoryId | Yes | getComparisonTemplate | Comparison template |
| getComparisonRows | $compId | Yes | getComparisonRows | Comparison rows |
| ... | ... | ... | ... | ... |

**STOP:** Do not proceed until you have mapped ALL functions from compare-related PHP files.
...
|--------------|--------------|-------------|
| GET /compare | GET /compare | Compare products |
| GET /comp/:id | GET /comp/:id | Comparison by ID |

### Entities to Use (from libs/database/src/entities/)

- `NCompClienta` - Client comparisons
- `NCompRow` - Comparison rows
- `NCompTemplate` - Comparison templates
- `Parts` - Products to compare
...
### Target Location
- Module: `apps/gateway/src/modules/compare/`
...
3. **DTOs**: Create validated DTOs for comparison data
4. **Caching**: Add caching for comparison templates
5. **Session/Auth**: Support both anonymous and authenticated comparisons
6. **Tests**: Unit tests with >80% coverage

### Key PHP Functions to Implement

From compare-related PHP files:
- `getComparison($productIds)` - Compare products [DB]
- `getComparisonById($id)` - Comparison by ID [DB]
- `createComparison($userId, $productIds)` - Create comparison [DB]
- `addToComparison($compId, $productId)` - Add product [DB]
- `removeFromComparison($compId, $productId)` - Remove product [DB]
- `getComparisonTemplate($categoryId)` - Template [DB]
- `getComparisonRows($compId)` - Comparison rows [DB]
...
### Validation Commands
```bash
npx nx test gateway --testPathPattern="compare"
