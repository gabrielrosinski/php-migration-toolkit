...
| `output/database/modules/schema_products.json` | Full database schema reference |
...
target_files = ['files/item.php', 'routes/items/index.php', 'files/popular_items.php', 'files/bought_together_items.php', 'files/display_items.php', 'files/item_specifications.php']
...
target_files = ['item.php', 'items/index.php', 'popular_items', 'bought_together', 'display_items', 'item_specifications']

found_any = False
for r in data.get('routes', []):
    target = r.get('target_file', '')
    path = r.get('nestjs_path', '')
    if any(t in target for t in target_files) or '/item' in path.lower():
        found_any = True
...
- [ ] All routes that belong to Products
...
```
Read tool parameters:
  file_path: output/database/modules/schema_products.json
```
...
|-------------|------------|-----|---------------|--------------|-------|
| queryItem | $uin | Yes | getProductById | Parameterized queries | Main product query |
| getComplementaryProducts | $uin | Yes | getComplementaryProducts | XSS sanitization | Related products |
| getSimilarItem | $uinsql | Yes | getSimilarProducts | Parameterized queries | Similar products |
| buildBMS | $uins, $disPayments | Yes | buildBMS | - | Bundle management |
| getPayments | $uin | Yes | getPaymentOptions | - | Payment options |
| getForFree | $uin | Yes | getFreeItems | - | Free items |
| ... | ... | ... | ... | ... | ... |
...
|--------------|--------------|-------------|
| GET /item/:id | GET /item/:id | Product details |
| GET /items/:id/complementary | GET /items/:id/complementary | Complementary products |
| GET /items/:id/similar | GET /items/:id/similar | Similar products |
| GET /items/:id/buy-together | GET /items/:id/buy-together | Frequently bought together |
| GET /items/:id/viewed-by-others | GET /items/:id/viewed-by-others | Others also viewed |
| GET /items/:id/stock/:storeId | GET /items/:id/stock/:storeId | Stock by store |
| GET /item_specifications | GET /item_specifications | Product specifications |
| GET /display_items | GET /display_items | Display items |
| GET /popular_items/:cat | GET /popular_items/:cat | Popular by category |
| GET /bought_together_items/:id | GET /bought_together_items/:id | Bought together |

### Entities to Use

- `Parts` - Main product entity
- `Part1` - Extended product data
- `PartsCache` - Product cache
- `PartPrices` - Price history
- `NtagParts` - Product-tag relationships
...
### Target Location
- Module: `apps/gateway/src/modules/products/`
- Project: `m-action-nestjs`

### Implementation Requirements

1. **Service Methods**: Implement ALL PHP functions from Phase 1 mapping (80+)
...
### Validation Commands
```bash
npx nx test gateway --testPathPattern="products"
