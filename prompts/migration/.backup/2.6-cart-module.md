...
| `output/analysis/architecture_security_db.json` | Security issues | XSS, SQL injection issues to fix |
...
| File | Purpose |
|------|---------|
| `output/database/entities/n-buy-items.entity.ts` | Cart items |
...
target_files = ['routes/set/addToCart/index.php', 'routes/set/addToCartUinsql/index.php', 'files/cart.php', 'files/buy.php', 'routes/cart/index.php']
...
target_files = ['addToCart', 'cart', 'buy']

for r in data.get('routes', []):
    target = r.get('target_file', '')
    path = r.get('nestjs_path', '')
    if any(t in target for t in target_files) or 'cart' in path.lower():
...
- [ ] All routes that belong to cart
...
```
Read tool parameters:
  file_path: output/database/modules/schema_cart.json
```
...
|-------------|------------|-----|---------------|----------------|
| addToCart | $uin, $qty, $idc | Yes | addToCartByUin | Add item by UIN |
| addToCartBySku | $sku, $qty, $idc | Yes | addToCartBySku | Add item by SKU |
| getCart | $idc | Yes | getCart | Get cart items |
| updateCartItem | $itemId, $qty | Yes | updateCartItem | Update quantity |
| removeFromCart | $itemId, $idc | Yes | removeFromCart | Remove item |
| clearCart | $idc | Yes | clearCart | Clear entire cart |
| getCartTotal | $idc | Yes | getCartTotal | Cart total count |
| getCartSummary | $idc | Yes | getCartSummary | Cart price summary |
| ... | ... | ... | ... | ... |

**STOP:** Do not proceed until you have mapped ALL functions from cart-related PHP files.
...
|--------------|--------------|-------------|
| GET /set/addToCart/:uin | GET /set/addToCart/:uin | Add item to cart by UIN |
| GET /set/addToCartUinsql/:sku | GET /set/addToCartUinsql/:sku | Add item to cart by SKU |
...
### Target Location
- Module: `apps/gateway/src/modules/cart/`
...
3. **DTOs**: Create validated DTOs for cart operations
4. **Session/Auth**: Integrate with JWT auth for user identification (idc)
5. **Caching**: Add caching for cart operations
6. **Tests**: Unit tests with >80% coverage

### Key PHP Functions to Implement

From cart-related PHP files:
- `addToCart($uin, $qty, $idc)` - Add by UIN [DB]
- `addToCartBySku($sku, $qty, $idc)` - Add by SKU [DB]
- `getCart($idc)` - Get cart items [DB]
- `updateCartItem($itemId, $qty)` - Update quantity [DB]
- `removeFromCart($itemId, $idc)` - Remove item [DB]
- `clearCart($idc)` - Clear cart [DB]
- `getCartTotal($idc)` - Total count [DB]
- `getCartSummary($idc)` - Price summary [DB]
...
### Validation Commands
```bash
npx nx test gateway --testPathPattern="cart"
npx nx build gateway
```

---

## PHASE 5: UPDATE MIGRATION SUMMARY

**MANDATORY:** After implementation, update `output/MIGRATION_SUMMARY.md` with:

1. **Module name and date**
2. **Mandatory Documents Read table** - confirm all Read tool calls
3. **Analysis Commands Executed** - confirm all bash extractions
4. **Function Mapping Table** - complete PHP to NestJS mapping
5. **Routes Implemented** - all routes with methods
6. **Security Requirements Applied** - with file:line references
7. **Files Created** - directory tree of new files

Use the template at the bottom of `MIGRATION_SUMMARY.md`.

---

## COMPLETION

When all verifications pass AND migration summary is updated:
```
SERVICE_COMPLETE
```
