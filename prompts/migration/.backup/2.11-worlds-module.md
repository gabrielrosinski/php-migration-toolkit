...
| File | Purpose |
|------|---------|
| `output/database/entities/api-world.entity.ts` | World configurations |
| `output/database/entities/z-main.entity.ts` | Main category/world data |
| `output/database/entities/z-item.entity.ts` | World items |
...
target_files = ['routes/world/index.php', 'routes/worlds/index.php', 'routes/new-world/index.php', 'routes/top/index.php', 'files/world.php']
...
target_files = ['world', 'worlds', 'new-world', 'top']

for r in data.get('routes', []):
    target = r.get('target_file', '')
    path = r.get('nestjs_path', '')
    if any(t in target for t in target_files) or any(t in path.lower() for t in ['world', 'top']):
...
- [ ] All routes that belong to worlds
...
```
Read tool parameters:
  file_path: output/database/modules/schema_worlds.json
```
...
|-------------|------------|-----|---------------|----------------|
| getWorldById | $id | Yes | getWorldById | World by ID |
| getAllWorlds | - | Yes | getAllWorlds | Get all worlds |
| getNewWorld | $id | Yes | getNewWorld | New world format |
| getTopWorlds | - | Yes | getTopWorlds | Top worlds/categories |
| getWorldBySelect | $select | Yes | getWorldBySelect | World by select param |
| getWorldCategories | $worldId | Yes | getWorldCategories | World categories |
| getWorldProducts | $worldId, $limit | Yes | getWorldProducts | Products in world |
| ... | ... | ... | ... | ... |

**STOP:** Do not proceed until you have mapped ALL functions from world-related PHP files.
...
|--------------|--------------|-------------|
| GET /world/:id | GET /world/:id | World by ID |
| GET /worlds | GET /worlds | All worlds |
| GET /new-world/:id | GET /new-world/:id | New world format |
| GET /top | GET /top | Top worlds/categories |

### Entities to Use (from libs/database/src/entities/)

- `ApiWorld` - World configurations
- `ZMain` - Main category/world data
- `ZItem` - World items
...
### Target Location
- Module: `apps/gateway/src/modules/worlds/`
...
3. **DTOs**: Create validated DTOs for world data
4. **Caching**: Add caching for world information
5. **Hierarchical Data**: Handle world/category hierarchy
6. **Tests**: Unit tests with >80% coverage

### Key PHP Functions to Implement

From world-related PHP files:
- `getWorldById($id)` - World by ID [DB]
- `getAllWorlds()` - All worlds [DB]
- `getNewWorld($id)` - New world format [DB]
- `getTopWorlds()` - Top worlds [DB]
- `getWorldBySelect($select)` - World by select [DB]
- `getWorldCategories($worldId)` - World categories [DB]
- `getWorldProducts($worldId, $limit)` - World products [DB]
...
### Validation Commands
```bash
npx nx test gateway --testPathPattern="worlds"
