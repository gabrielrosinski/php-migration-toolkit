...
| `output/database/modules/schema_search.json` | Full database schema reference |
...
target_files = ['files/search.php', 'products/search/autocomplete/index.php']
...
target_files = ['search.php', 'autocomplete']

found_any = False
for r in data.get('routes', []):
    target = r.get('target_file', '')
    path = r.get('nestjs_path', '')
    if any(t in target for t in target_files) or 'search' in path.lower():
        found_any = True
...
- [ ] All routes that belong to Search
...
```
Read tool parameters:
  file_path: output/database/modules/schema_search.json
```
...
|-------------|------------|-----|---------------|--------------|-------|
| getHotTags | $lang | Yes | getHotTags | Parameterized queries | Trending tags |
| getHotSearch | - | Yes | getHotSearches | - | Popular searches |
| getPersonalItem | $idc | Yes | getPersonalizedItems | XSS sanitization | User-specific items |
| cln | $q | No | cleanQuery | Input sanitization | Clean search query |
| send_to_api | $url, $query, ... | Yes | sendToSearchApi | - | External API call |
| clearSearch | ... | No | sanitizeSearch | XSS sanitization | Input sanitization |
| ... | ... | ... | ... | ... | ... |
...
|--------------|--------------|-------------|
| GET /searchPage | GET /searchPage | Full search page with filters |
| GET /products/search/autocomplete/:query | GET /products/search/autocomplete/:query | Search autocomplete |

### Entities to Use

- `Parts` - Product search
- `NSearchH` - Search history
- `NSearchNormalize` - Search normalization
...
### Target Location
- Module: `apps/gateway/src/modules/search/`
...
### Validation Commands
```bash
npx nx test gateway --testPathPattern="search"
